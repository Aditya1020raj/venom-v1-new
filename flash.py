...
FORWARD_CHANNEL_ID = --1002330296790
CHANNEL_ID = -1002330296790
error_channel_id = -1002330296790
...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*You are not authorized to use this command ye video dekh kr sikh le pehle https://t.me/flashmainchannel/79 *", parse_mode='Markdown')
        return

    if len(cmd_parts) < 2:
        bot.send_message(chat_id, '''*theek se krUse 
/approve <PS I'd> 2 99 
        or
/disapprove <PS_id>.*''', parse_mode='Markdown')
        return
...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "You are not approved to use this bot. Please watch this video 👉 https://t.me/flashmainchannel/79 .")
            return
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*You are not approved to use this bot. Please watch this video 👉 https://t.me/flashmainchannel/79.*", parse_mode='Markdown')
            return
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*CHIN TAPAK DUM DUM  💥\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration} FEEDBACK BHEJ DIO YAAD SE 😡*", parse_mode='Markdown')
    except Exception as e:
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.reply_to(message, '''*👇Channel I'd yha milegi👇
@GetChatID_IL_BOT
👇Approval krne ke liye I'd yha milegi 👇
@username_to_id_bot
        *''', parse_mode='Markdown')
